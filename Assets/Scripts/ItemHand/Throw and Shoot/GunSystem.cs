...
public class GunSystem : MonoBehaviour, ItemHand
{
    ...
    {
        if (ObjectsDatabase.singleton.playerStatus.GetMana < weaponData._manaConsumption) return;
            ...
            if (timer >= weaponData._timeBetweenShots && animValue == 1)
            {
                if (shootAgain)
                {
                    ...
                    */
                    ...
                    ObjectsDatabase.singleton.playerStatus.ConsumeMana(weaponData._manaConsumption);
